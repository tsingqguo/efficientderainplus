...
class DenoisingDataset(Dataset):
    ...
    def rain_aug(self, img_rainy, img_gt):
        ...
        rainlayer_aug2 = augment_and_mix.augment_and_mix(rainlayer_rand2, severity = 3, width = 4, depth = -1) * 1
    ...
    def rain_augv2(self, img_rainy, img_gt):
        if random.randint(0, 10) > 7:
            return img_rainy, img_gt
        img_rainy = (img_rainy.astype(np.float32)) / 255.0
        img_gt = (img_gt.astype(np.float32)) / 255.0
        #img_rainlayer = (img_rainy - img_gt) / (1 - img_gt)
        img_rainlayer = img_rainy - img_gt
        if random.randint(0, 10) > 7:
            img_gt = augment_and_mix.augment_and_mix(img_gt, severity = 3, width = 3, depth = -1) * 1
        if random.randint(0, 10) > 7:
            img_rainlayer = augment_and_mix.augment_and_mix(img_rainlayer, severity = 3, width = 3, depth = -1) * 1
        #img_rainy = img_gt + img_rainlayer - img_gt*img_rainlayer
        img_rainy = img_gt + img_rainlayer
        
        img_rainy = img_rainy*255
        img_gt = img_gt*255
        return img_rainy, img_gt
    ...
    def __getitem__(self, index):
        ...
        #print(self.imglist[index][2])
        
        if self.rainaug:
            img_rainy, img_gt = self.rain_augv2(img_rainy, img_gt)
        #cv2.imwrite("/mnt/nvme/jingyang/DERAIN2/code_baseline_rainmixv2.0/tmp/" + self.imglist[index][2] + "_rainy.jpg", cv2.cvtColor(img_rainy, cv2.COLOR_RGB2BGR))
        #cv2.imwrite("/mnt/nvme/jingyang/DERAIN2/code_baseline_rainmixv2.0/tmp/" + self.imglist[index][2] + "_gt.jpg", cv2.cvtColor(img_gt, cv2.COLOR_RGB2BGR))
