...
class KPNP(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        self.KPN1 = KPN(color=color, burst_length=burst_length, blind_est=blind_est, kernel_size=kernel_size, sep_conv=sep_conv, channel_att=channel_att, spatial_att=spatial_att, upMode=upMode, core_bias=core_bias)
        self.KPN2 = KPN(color=color, burst_length=burst_length, blind_est=blind_est, kernel_size=kernel_size, sep_conv=sep_conv, channel_att=channel_att, spatial_att=spatial_att, upMode=upMode, core_bias=core_bias)
    
    def forward(self, data_with_est, data, white_level=1.0):
        b1 = self.KPN1(data_with_est, data)
        pred = self.KPN2(b1, b1)
        return pred
