...
def create_generator(opt):
    ...
    generator = network.KPNP(opt.color, opt.burst_length, opt.blind_est, opt.kernel_size, opt.sep_conv, \
        opt.channel_att, opt.spatial_att, opt.upMode, opt.core_bias)
    if opt.load_name == '':
        # Init the network
        network.weights_init(generator, init_type = opt.init_type, init_gain = opt.init_gain)

        pretrained_net = torch.load("./models_L49_K3D1D2D3D4_3x3_rainmixW3DRF_SPA/KPN_rainy_image_epoch17_bs16.pth")
        load_dict(generator.KPN1, pretrained_net)
    ...
    if "new" in path:
        ...
        for root, dirs, files in os.walk(path_gt):
            ...
            for name in files:
                if name.split('.')[1] != 'png':
                    continue
                name_p0 = name.split('.')[0]
                file_rainy = path_rainy + "/" + name_p0 + "x2.png"
                file_gt = path_gt + "/" + name
                ret.append([file_rainy, file_gt])        
    else:
        path_rainy = path + "/small/rain"
