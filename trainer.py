...
def Pre_train(opt):
    ...
    for epoch in range(opt.epochs):
        for i, (true_input, true_target) in enumerate(train_loader):
            ...
            b1, b2, fake_target = generator(true_input, true_input)
            
            
            ssim_loss_b = -criterion_ssim(true_target, fake_target)
            ssim_loss_b1 = -criterion_ssim(true_target, b1)
            ssim_loss_b2 = -criterion_ssim(true_target, b2)

            # L1 Loss
            Pixellevel_L1_Loss_b = criterion_L1(fake_target, true_target)
            Pixellevel_L1_Loss_b1 = criterion_L1(b1, true_target)
            Pixellevel_L1_Loss_b2 = criterion_L1(b2, true_target)

            loss_b = ssim_loss_b + Pixellevel_L1_Loss_b
            loss_b1 = ssim_loss_b1 + Pixellevel_L1_Loss_b1
            loss_b2 = ssim_loss_b2 + Pixellevel_L1_Loss_b2
            ...
            loss = 0.6*loss_b + 0.2*loss_b1 + 0.2*loss_b2
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [Loss: %.4f %.4f %.4f %.4f %.4f %.4f] Time_left: %s" %
                ((epoch + 1), opt.epochs, i, len(train_loader), Pixellevel_L1_Loss_b.item(), ssim_loss_b.item(), Pixellevel_L1_Loss_b1.item(), ssim_loss_b1.item(), Pixellevel_L1_Loss_b2.item(), ssim_loss_b2.item(), time_left))
