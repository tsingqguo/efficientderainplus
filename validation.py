...
#    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    ...
    psnr_gray_sum, psnr_gray_ave, ssim_gray_sum, ssim_gray_ave, eval_gray_cnt = 0, 0, 0, 0, 0
    ...
    for i, (true_input, true_target, height_origin, width_origin) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            _, _, fake_target = generator(true_input, true_input)
        ...
        img_pred_gray_recover = utils.rgb2ycbcr(cv2.cvtColor(img_pred_recover, cv2.COLOR_BGR2RGB)/255)*255
        img_gt_gray_recover = utils.rgb2ycbcr(cv2.cvtColor(img_gt_recover, cv2.COLOR_BGR2RGB)/255)*255
        ...
        psnr_gray_sum = psnr_gray_sum + utils.psnr(img_pred_gray_recover, img_gt_gray_recover)
        ssim_gray_sum = ssim_gray_sum + compare_ssim(img_gt_gray_recover, img_pred_gray_recover, multichannel = False, data_range = 255, gaussian_weights = True) 
    ...
    psnr_gray_ave = psnr_gray_sum / eval_cnt
    ssim_gray_ave = ssim_gray_sum / eval_cnt
    psnr_file = "./data/psnr_data.txt"
    ssim_file = "./data/ssim_data.txt"
    
    psnr_content = "BGR: " + str(psnr_ave) + "; ycbcr-Y: " + str(psnr_gray_ave) + "  " + opt.load_name + "\n"
    ssim_content = "BGR: " + str(ssim_ave) + "; ycbcr-Y: " + str(ssim_gray_ave) + "  " + opt.load_name + "\n"
