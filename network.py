...
class KPN1(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        # 各个卷积层定义
        # 2~5层都是均值池化+3层卷积
        self.conv1 = Basic1(in_channel, 64, channel_att=False, spatial_att=False)
        self.conv2 = Basic1(64, 128, channel_att=False, spatial_att=False)
        self.conv3 = Basic1(128, 256, channel_att=False, spatial_att=False)
        self.conv4 = Basic1(256, 512, channel_att=False, spatial_att=False)
        self.conv5 = Basic1(512, 512, channel_att=False, spatial_att=False)
        # 6~8层要先上采样再卷积
        self.conv6 = Basic1(512+512, 512, channel_att=channel_att, spatial_att=spatial_att)
        self.conv7 = Basic1(256+512, 256, channel_att=channel_att, spatial_att=spatial_att)
        self.conv8 = Basic1(256+128, out_channel, channel_att=channel_att, spatial_att=spatial_att)
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        core_pool = F.avg_pool2d(core.view(core.shape[0], 3, core.shape[1]//3, core.shape[2]*core.shape[3]), (9,1)).view(core.shape[0], 3, core.shape[2], core.shape[3])    
        return pred, core_pool
...
class KPN2(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
        ...
        # 各个卷积层定义
        # 2~5层都是均值池化+3层卷积
        self.conv1 = Basic2(in_channel, 64, channel_att=False, spatial_att=False)
        self.conv2 = Basic2(64, 128, channel_att=False, spatial_att=False)
        self.conv3 = Basic2(128, 256, channel_att=False, spatial_att=False)
        self.conv4 = Basic2(256, 512, channel_att=False, spatial_att=False)
        self.conv5 = Basic2(512, 512, channel_att=False, spatial_att=False)
        # 6~8层要先上采样再卷积
        self.conv6 = Basic2(512+512, 512, channel_att=channel_att, spatial_att=spatial_att)
        self.conv7 = Basic2(256+512, 256, channel_att=channel_att, spatial_att=spatial_att)
        self.conv8 = Basic2(256+128, out_channel, channel_att=channel_att, spatial_att=spatial_att)
        self.outc = nn.Conv2d(out_channel, out_channel, 1, 1, 0)
    ...
    def forward(self, data_with_est, data, white_level=1.0):
        ...
        #core_pool = F.avg_pool2d(core.view(core.shape[0], 1, core.shape[1]//3, core.shape[2]*core.shape[3]*3), (9,1)).view(core.shape[0], 3, core.shape[2], core.shape[3])    
        #return pred, core_pool
        return pred

class KPNP(nn.Module):
    def __init__(self, color=True, burst_length=1, blind_est=True, kernel_size=[5], sep_conv=False,
                 channel_att=False, spatial_att=False, upMode='bilinear', core_bias=False):
        super(KPNP, self).__init__()
        self.KPN1 = KPN1(color=color, burst_length=burst_length, blind_est=blind_est, kernel_size=kernel_size, sep_conv=sep_conv, channel_att=channel_att, spatial_att=spatial_att, upMode=upMode, core_bias=core_bias)
        self.KPN2 = KPN2(color=color, burst_length=burst_length, blind_est=blind_est, kernel_size=kernel_size, sep_conv=sep_conv, channel_att=channel_att, spatial_att=spatial_att, upMode=upMode, core_bias=core_bias)
        self.s1 = nn.Conv2d(in_channels=6, out_channels=3, kernel_size=1, stride=1, padding=0)

    def forward(self, data_with_est, data, white_level=1.0):
        b1, pooling = self.KPN1(data_with_est, data)
        x = torch.cat([b1, pooling], dim=1)
        b2 = self.KPN2(x, b1)
        b1c2 = torch.cat([b1, b2], dim=1)
        pred = self.s1(b1c2)
        return b1, b2, pred
